...
export default {
  install: function (_app: App) {
    let name = ''
    for (const key in tsxComp) {
      name = tsxComp[key].default.name || key.substring(key.lastIndexOf('/') + 1)
      name = name.replace('.tsx', '')
      _app.component(name, tsxComp[key].default)
    }
    for (const key in vueComp) {
      name = vueComp[key].default.name || key.substring(key.lastIndexOf('/') + 1)
